...
const Comida = () => {
  ...
  const guardarGastoAnterior = async nuevoGasto => {
    try {
      const gastosGuardados = await AsyncStorage.getItem('gastosAnteriores');
  ...
  const handleResetearTotal = async () => {
    try {
      ...
      await AsyncStorage.removeItem('gastosAnteriores');
      setTotalGastado(0);
      setGastosAnteriores([]);
    } catch (error) {
  ...
  return (
    <View style={styles.container}>
      <Text style={styles.title}>Registrar Gasto de Comida</Text>
